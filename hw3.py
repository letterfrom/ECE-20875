...
def norm_histogram(histogram):
    ...
    total = counts.sum()
    # Guard against division by zero; if empty, return zeros of same shape
    if total == 0:
...
def get_coordinates(data, each_dist):
    ...
    """
    # Use scipy.stats.probplot to obtain theoretical quantiles (X) and ordered sample (Y)
    # We do not fit a line (fit=False); we pass the dist by name or object.
    dist_obj = getattr(stats, each_dist) if isinstance(each_dist, str) else each_dist
    (osm, osr), _ = stats.probplot(np.asarray(data), dist=dist_obj, fit=False)
    X = np.asarray(osm, dtype=float)
    Y = np.asarray(osr, dtype=float)
    return X, Y
...
def calculate_distance(x, y):
    ...
    :return: float
    """
    # Perpendicular distance from point (x, y) to the 45-degree line y = x
    return float(abs(y - x) / np.sqrt(2.0))
...
def find_dist(data):
    ...
    :return: (str, float)
    """
    # Return (name_with_min_error, min_error)
    name = min(data, key=lambda k: data[k])
